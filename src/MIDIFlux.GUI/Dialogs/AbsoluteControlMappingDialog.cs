...
{
    /// <summary>
    /// Dialog for editing an absolute control mapping
    /// </summary>
    public partial class AbsoluteControlMappingDialog : BaseDialog
    {
        private readonly ILogger _logger;
        private readonly AbsoluteControlMapping _mapping;
        ...
        /// <summary>
        /// Gets the edited absolute control mapping
        /// </summary>
        public AbsoluteControlMapping Mapping => _mapping;

        /// <summary>
        /// Initializes a new instance of the <see cref="AbsoluteControlMappingDialog"/> class for creating a new mapping
        /// </summary>
        /// <param name="midiManager">Optional MidiManager for MIDI listening functionality</param>
        public AbsoluteControlMappingDialog(MidiManager? midiManager = null) : this(new AbsoluteControlMapping { ControlNumber = 7, HandlerType = "SystemVolume" }, midiManager) // Default to CC 7 (Volume) and SystemVolume handler
        {
            _isNewMapping = true;
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="AbsoluteControlMappingDialog"/> class for editing an existing mapping
        /// </summary>
        /// <param name="mapping">The absolute control mapping to edit</param>
        /// <param name="midiManager">Optional MidiManager for MIDI listening functionality</param>
        public AbsoluteControlMappingDialog(AbsoluteControlMapping mapping, MidiManager? midiManager = null)
        {
            // Create logger
            _logger = LoggingHelper.CreateLogger<AbsoluteControlMappingDialog>();
            _logger.LogDebug("Initializing AbsoluteControlMappingDialog");
            ...
            Text = _isNewMapping ? "Add Absolute Control Mapping" : "Edit Absolute Control Mapping";
        ...
        private void LoadMappingData()
        {
            ApplicationErrorHandler.RunWithUiErrorHandling(() =>
            {
                try
                {
                    ...
                    handlerTypeComboBox.Items.Add("SystemVolume");
                    handlerTypeComboBox.Items.Add("GameControllerAxis");
                    handlerTypeComboBox.Items.Add("CCRange");
                    ...
                    else
                    {
                        ...
                        _mapping.HandlerType = handlerTypeComboBox.SelectedItem?.ToString() ?? "SystemVolume";
                    }
            ...
            }, _logger, "loading absolute control mapping data", this);
        }
        ...
        private void UpdateUIForHandlerType()
        {
            ...
            ApplicationErrorHandler.RunWithUiErrorHandling(() =>
            {
                try
                {
                    ...
                    string handlerType = handlerTypeComboBox.SelectedItem?.ToString() ?? "SystemVolume";
                    ...
                    string description = handlerType switch
                    {
                        ...
                        "CCRange" => "Maps CC value ranges to different actions",
        ...
        private bool SaveMappingData()
        {
            ...
            {
                _logger.LogError(ex, "Error saving absolute control mapping data");
        ...
        private bool ValidateMapping()
        {
            ...
            {
                _logger.LogError(ex, "Error validating absolute control mapping data");
        ...
        private void TestButton_Click(object? sender, EventArgs e)
        {
            ...
            }, _logger, "testing absolute control mapping", this);
        }
        ...
        private void okButton_Click(object sender, EventArgs e)
        {
            ...
            }, _logger, "saving absolute control mapping", this);
        }
