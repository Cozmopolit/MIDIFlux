...
/// <summary>
/// Base class for asynchronous actions that encapsulates common boilerplate.
/// Handles logger initialization, error handling, and provides true async execution.
/// Derived classes implement ExecuteAsyncCore() for their async business logic.
/// </summary>
/// <typeparam name="TConfig">The strongly-typed configuration class for this action</typeparam>
public abstract class AsyncActionBase<TConfig> : IAction where TConfig : ActionConfig
{
    ...
    protected AsyncActionBase(TConfig config)
    {
    ...
    public async ValueTask ExecuteAsync(int? midiValue = null)
    {
        try
        {
            Logger.LogDebug("Executing {ActionType} (async): {Description}, MidiValue={MidiValue}",
                GetType().Name, Description, midiValue);

            // Call the derived class implementation
            await ExecuteAsyncCore(midiValue);

            Logger.LogTrace("Successfully executed {ActionType} (async): {Description}", GetType().Name, Description);
        }
        catch (Exception ex)
        {
